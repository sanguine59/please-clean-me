...
public class Main {
	...
	private void showUpgradeMenu() {
		...
		do {
			...
			System.out.println("1. Increase Restaurant's Seat (Rp. " + game.seatExpansionCost() + ')');
	...
	private void showHireMenu() {
		...
		do {
			displayCurrentStatus();
			System.out.println("1. Hire New Waiter (Rp. " + game.waiterHireCost() + ") ");
			System.out.println("2. Hire New Chef (Rp. " + game.chefHireCost() + ") ");
	...
	private void showUpgradeChefMenu() {
		...
		do {
			...
			if (choice > 0 && choice <= chefs.size()) {
				...
				ChefStat stat = ChefStat.fromString(raw);
				if (stat == null) {
					System.out.println("Upgrade failed. Invalid stat.");
				} else if (game.upgradeChef(choice - 1, stat)) {
					System.out.println("Upgrade successful!");
				} else {
					System.out.println("Upgrade failed. Not enough money or chef is max level.");
				}
			}
		} while (choice != 0);
	}
